...
public abstract class AbstractDemoChart implements IDemoChart {
  ...
  protected XYMultipleSeriesDataset buildDataset( List<String> titles,  List<List<Double>> xValues,
	      List<List<Double>> yValues) {
	    XYMultipleSeriesDataset dataset = new XYMultipleSeriesDataset();
	    addXYSeries(dataset, titles, xValues, yValues, 0);
	    return dataset;
	  }
  public void addXYSeries(XYMultipleSeriesDataset dataset, List<String> titles, List<List<Double>> xValues,
	      List<List<Double>> yValues, int scale) {
   ...
   * @return the category series
   */
  protected CategorySeries buildCategoryDataset(String title,List<String> categorys, List<Double> values) {
    CategorySeries series = new CategorySeries(title);
    for (int i=0;i<categorys.size();i++) {
    	double value = values.get(i);
    	if(value>0){
    		series.add(categorys.get(i),value);
    	}
    }
    return series;
  }
   ...
   */
  protected DefaultRenderer buildCategoryRenderer(List<Integer> colors) {
